...
export class ReviewNotificationPolicy {
    ...
    buildForSeller(
        ...
        const mediaText =
            event.data.mediaCount > 0
                ? ` Có ${event.data.mediaCount} tệp đính kèm.`
                : '';
        const commentText = event.data.hasComment
        ...
        return {
            ...
            type: isUpdated
                ? 'product_review_updated_for_seller'
                : 'product_review_created_for_seller',
            audiences: [
                {
            ...
            title: isUpdated
                ? 'Đánh giá sản phẩm đã được cập nhật'
