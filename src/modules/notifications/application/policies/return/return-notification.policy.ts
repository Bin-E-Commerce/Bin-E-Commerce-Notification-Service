...
export class ReturnNotificationPolicy {
    ...
    buildForSeller(event: ReturnChangedEvent): CreateNotificationInput | null {
        return event.data.sellerUserId
            ? this.build(event, event.data.sellerUserId, 'seller')
            : null;
    }
    ...
    private build(
        ...
        return {
            ...
            badgeKey:
                audience === 'seller' ? 'seller.returns' : 'customer.orders',
