...
import { buildOtpTemplate } from '@/modules/email/application/templates/auth/otp.template';
import { buildUserStatusChangedTemplate } from '@/modules/email/application/templates/auth/user-status-changed.template';
import { buildUserRoleChangedTemplate } from '@/modules/email/application/templates/auth/user-role-changed.template';
import { buildSellerApplicationSubmittedTemplate } from '@/modules/email/application/templates/seller-applications/seller-application-submitted.template';
import { buildSellerApplicationRejectedTemplate } from '@/modules/email/application/templates/seller-applications/seller-application-rejected.template';
import { buildSellerApplicationApprovedTemplate } from '@/modules/email/application/templates/seller-applications/seller-application-approved.template';
import {
    buildOrderCancelledTemplate,
    buildOrderCreatedTemplate,
    type OrderEmailItem,
    type OrderEmailRole,
} from '@/modules/email/application/templates/orders/order.template';
import {
    buildShipmentStatusTemplate,
    type ShipmentEmailRole,
} from '@/modules/email/application/templates/shipments/shipment.template';
import {
    buildReturnStatusTemplate,
    type ReturnEmailRole,
} from '@/modules/email/application/templates/returns/return.template';
...
export class EmailService {
    ...
    constructor(private readonly config: ConfigService) {
        ...
        this.webBaseUrl = this.resolveWebBaseUrl();
        this.logoPath = resolve(
            this.config.get<string>(
                'EMAIL_LOGO_PATH',
                resolve(
                    process.cwd(),
                    'assets/email/logo_background_white.png',
                ),
            ),
        );
    ...
    // Chuẩn hóa URL public dùng trong email và fail fast ở production nếu cấu hình
    // vẫn trỏ về localhost; gửi email với link nội bộ sẽ tạo trải nghiệm hỏng cho người dùng.
    private resolveWebBaseUrl(): string {
        const nodeEnv = this.config.get<string>('NODE_ENV', 'development');
        const configuredUrl = this.config.get<string>('WEB_BASE_URL')?.trim();
        const webBaseUrl = configuredUrl || 'http://localhost:5173';

        let parsedUrl: URL;
        try {
            parsedUrl = new URL(webBaseUrl);
        } catch {
            throw new Error('WEB_BASE_URL must be a valid absolute URL');
        }
        ...
        if (
            nodeEnv === 'production' &&
            (isLocalHost || parsedUrl.protocol !== 'https:')
        ) {
            throw new Error(
                'WEB_BASE_URL must be a public HTTPS URL in production; localhost is not allowed',
            );
        }
    ...
    async sendOtpEmail(
        ...
        try {
            ...
            this.logger.error(
                `Failed to send OTP email to ${to}: ${String(err)}`,
            );
            throw err;
        }
    }

    // Gửi email ban/gỡ ban theo snapshot của event; nội dung nêu rõ trạng thái, lý do và thời điểm
    // để người dùng biết vì sao không đăng nhập được hoặc có thể quay lại hệ thống.
    async sendUserStatusChangedEmail(input: {
        to: string;
        name: string;
        status: UserStatus;
        reason: string;
        ...
        const attachments = this.buildBrandAttachments();
        const template = buildUserStatusChangedTemplate({
            ...input,
    ...
    // Gửi email role mới sau khi Auth Service đã thu hồi session cũ, giúp user hiểu vì sao phải đăng nhập lại.
    async sendUserRoleChangedEmail(input: {
        to: string;
        name: string;
        ...
        const attachments = this.buildBrandAttachments();
        const template = buildUserRoleChangedTemplate({
            ...input,
            webBaseUrl: this.webBaseUrl,
            logoCid: attachments.length > 0 ? this.logoCid : undefined,
        });
    ...
    private buildBrandAttachments(): NonNullable<
    ...
    private async sendTemplateEmail(
        ...
        try {
            ...
            this.logger.error(
                `Failed to send ${label} email to ${to}: ${String(err)}`,
            );
            throw err;
        }
    }
