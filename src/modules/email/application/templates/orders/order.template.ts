...
export function buildOrderCreatedTemplate(
    ...
    const createdAt = escapeEmailHtml(
        formatVietnameseDateTime(input.createdAt),
    );
    ...
    const itemsHtml =
        !isSeller && input.items?.length
            ? `<h2 style="margin:24px 0 10px;font-family:Arial,sans-serif;font-size:17px;line-height:24px;font-weight:700;color:#18181b;">Sản phẩm đã đặt</h2><table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="width:100%;border-top:1px solid #e4e4e7;border-bottom:1px solid #e4e4e7;">${input.items
                  .map((item) => {
                      ...
                      const variant = item.variantName
                          ? ` · ${escapeEmailHtml(item.variantName)}`
                          : '';
    ...
    const itemsText =
        !isSeller && input.items?.length
            ? [
                  ...
                  ...input.items.map(
                      (item) =>
                          `- ${item.productName}${item.variantName ? ` (${item.variantName})` : ''} · SL: ${item.quantity} · ${item.lineTotal} đ`,
                  ),
              ]
            : [];
    ...
    return {
        subject: sanitizeEmailSubject(
            `[Bin] ${roleTitle} · ${input.orderNumber}`,
        ),
...
export function buildOrderCancelledTemplate(
    input: OrderEmailInput,
): EmailTemplate {
    const safeOrderNumber = escapeEmailHtml(input.orderNumber);
    const safeOrderUrl = escapeEmailHtml(input.orderUrl);
    ...
    return {
        subject: sanitizeEmailSubject(
            `[Bin] Đơn hàng đã hủy · ${input.orderNumber}`,
        ),
        html: renderEmailLayout({
